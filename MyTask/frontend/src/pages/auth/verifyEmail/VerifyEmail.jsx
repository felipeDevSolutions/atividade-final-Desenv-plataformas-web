...
const VerifyEmail = () => {
  ...
  useEffect(() => {
    const checkVerificationStatus = async () => {
      ...
      try {
        const response = await axios.get(`https://mytask-ze7d.onrender.com/api/validate`, { // Verifica o status de login
          headers: {
  ...
  const resendVerificationEmail = async () => {
    setIsLoading(true);
    try {
      const response = await axios.post('https://mytask-ze7d.onrender.com/api/resend-verification', { uid });
      if (response.status === 200) {
